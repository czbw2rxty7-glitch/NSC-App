...
// Sends booking confirmation emails to parent and staff.
//
...
exports.handler = async (event) => {
  ...
  try {
    const { parentName, parentEmail, childName, childAge, sessions, totalPaid, paymentRef, medicalNotes } = JSON.parse(event.body);
    ...
    const sessionListText = sessions.map(s => `• ${s.name} — ${s.date} at ${s.time}, ${s.location}`).join("\n");
...
<table width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
  ...
  <tr><td style="background:#ffffff;padding:28px 32px;">
    ...
    <p style="font-size:15px;color:#475569;margin:0 0 24px;line-height:1.6;">
      Great news — <strong>${childName}</strong>'s booking is confirmed and their place is secured!
    </p>
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e2e8f0;border-radius:8px;overflow:hidden;margin-bottom:20px;">
      ...
      <tr style="background:#f8fafc;">
        <td style="padding:12px 14px;font-weight:700;color:#0d1b2a;">Total Paid</td>
        <td style="padding:12px 14px;text-align:right;font-weight:800;font-size:18px;color:#0d9488;">£${parseFloat(totalPaid).toFixed(2)}</td>
      </tr>
    </table>
    ...
    <div style="background:#f0fdfa;border:1px solid #99f6e4;border-radius:8px;padding:14px;margin-bottom:20px;">
      <p style="font-size:14px;color:#0f766e;margin:0;line-height:1.7;">
        ⚽ Please arrive 5 minutes before the session starts.<br>
        📧 To cancel, contact us at least 24 hours before.<br>
        📞 Questions? Reply to this email or visit nscperformance.co.uk
      </p>
    </div>
...
<div style="max-width:480px;background:#fff;border-radius:10px;padding:24px;border:1px solid #e2e8f0;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px;">
    ...
    <div>
      <h2 style="margin:0;color:#0d1b2a;font-size:18px;">New Booking Received</h2>
      <p style="margin:0;color:#64748b;font-size:12px;">${new Date().toLocaleString("en-GB", { timeZone: "Europe/London" })}</p>
    </div>
  </div>
  <table width="100%" cellpadding="0" cellspacing="0" style="margin-bottom:16px;">
    <tr><td style="padding:6px 0;color:#64748b;font-size:13px;width:110px;">Child</td><td style="font-weight:700;font-size:14px;color:#0d1b2a;">${childName} <span style="color:#64748b;font-weight:400;">(${childAge})</span></td></tr>
  ...
  <div style="background:#f8fafc;border-radius:8px;padding:14px;margin-bottom:14px;">
    ...
    ${sessions.map(s => `<div style="font-size:13px;padding:5px 0;border-bottom:1px solid #e2e8f0;color:#475569;"><strong style="color:#0d1b2a">${s.name}</strong> — ${s.date} at ${s.time}</div>`).join("")}
  </div>
  ${medicalNotes && medicalNotes.toLowerCase() !== "none" ? `
  <div style="background:#fef3c7;border:1px solid #fbbf24;border-radius:8px;padding:12px;">
    <strong style="color:#92400e;font-size:13px;">⚠ Medical Notes — please read before session:</strong>
    <p style="color:#92400e;margin:6px 0 0;font-size:13px;">${medicalNotes}</p>
  </div>` : ""}
</div>
</body>
</html>`;
    ...
    return reply(500, { error: "Email failed: " + err.message });
  }
};
